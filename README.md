<div align="center">

# Hello, I'm Bernardo Tomaz

### Full Stack Developer focused on backend development, APIs, relational databases and business-oriented systems.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Bernardo%20Tomaz-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/bernardo-tomaz-493a78136)
[![Portfolio](https://img.shields.io/badge/Portfolio-berndev.online-111111?style=for-the-badge&logo=google-chrome&logoColor=white)](https://berndev.online/)

</div>

---

## About Me

I'm a Full Stack Developer with a strong focus on backend development, APIs, relational databases and scalable business systems.

I work mainly with **C#**, **.NET**, **ASP.NET Core**, **Entity Framework**, **Angular**, **TypeScript**, **SQL Server** and **Oracle**.

I enjoy building reliable software that solves real problems: clean APIs, well-structured data models, maintainable code and systems that support real business workflows.

I'm currently deepening my knowledge in backend architecture, system design, relational databases and software engineering best practices through practical projects.

---

## What I Work With

- Backend development with **C#**, **.NET** and **ASP.NET Core**
- REST API design, business rules and service integration
- Relational database modeling with **SQL Server**, **Oracle** and **PostgreSQL**
- Frontend development with **Angular**, **TypeScript**, **HTML** and **CSS**
- Internal systems, dashboards and workflow-oriented applications
- Code organization, maintainability and practical software architecture

---

## Main Stack

<div align="left">

### Backend

![C#](https://img.shields.io/badge/C%23-68217A?style=for-the-badge&logo=csharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![ASP.NET Core](https://img.shields.io/badge/ASP.NET%20Core-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![Entity Framework](https://img.shields.io/badge/Entity%20Framework-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![LINQ](https://img.shields.io/badge/LINQ-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST%20APIs-02569B?style=for-the-badge&logo=swagger&logoColor=white)
![Swagger/OpenAPI](https://img.shields.io/badge/Swagger%20%2F%20OpenAPI-85EA2D?style=for-the-badge&logo=swagger&logoColor=black)

### Frontend

![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Angular Material](https://img.shields.io/badge/Angular%20Material-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### Databases

![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)

### Tools

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![TFS](https://img.shields.io/badge/TFS-0078D7?style=for-the-badge&logo=azuredevops&logoColor=white)
![GitHub Copilot](https://img.shields.io/badge/GitHub%20Copilot-111111?style=for-the-badge&logo=githubcopilot&logoColor=white)

</div>

---

## Also Studying and Using

<div align="left">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![SharePoint](https://img.shields.io/badge/SharePoint-038387?style=for-the-badge&logo=microsoftsharepoint&logoColor=white)

</div>

- **Python** for automation, scripting and data analysis.
- **Jupyter Notebook** for experiments, visualizations and machine learning studies.
- **Power BI** for dashboards, indicators and data visualization.
- **SharePoint** for internal tools and business process support.

---

## Current Focus

- Building REST APIs with **.NET** and **ASP.NET Core**
- Improving backend architecture and maintainability
- Designing relational databases, queries and data models
- Developing Angular applications connected to real business workflows
- Studying system design and software engineering practices through real projects

---

## Development Principles

- Write code that is clear before it is clever.
- Design APIs around real use cases, not only technical structure.
- Keep data models consistent, readable and easy to evolve.
- Prefer maintainable solutions over unnecessary complexity.
- Understand the business problem before choosing the technical solution.

---

## GitHub Activity

<div align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=BernTomaz&theme=tokyonight" width="700" />

</div>

---

## Let's Connect

I'm open to talking about technology, backend development, software architecture and projects that solve real business problems.

<div align="left">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Bernardo%20Tomaz-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/bernardo-tomaz-493a78136)
[![Portfolio](https://img.shields.io/badge/Portfolio-berndev.online-111111?style=for-the-badge&logo=google-chrome&logoColor=white)](https://berndev.online/)

</div>

---

> "Simplicity is a prerequisite. If simplicity is not perceived at the beginning, it will be impossible to achieve it later."
>
> - Edsger Dijkstra
